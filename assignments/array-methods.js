...
// Problem 1
// Each person gets a shirt with their company name and initials printed on it. Return an array of each runner's company name, initials, and shirt size to send to the screen printers.
...
// Problem 2
// Collect a list of everyone wearing a shirt size medium or smaller.

let shirtList = [];
runners.filter(function (runner) {
    if (runner.shirt_size === "M" || runner.shirt_size === "S" || runner.shirt_size === "XS") {
        shirtList.push(`${runner.first_name} ${runner.last_name}`);
    }
})
console.log(shirtList);

// Problem 3
// Get the number of shirts per each shirt size
